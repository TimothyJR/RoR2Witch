...
{
	class WindPrimary : BaseSkillState
	{
		...
		{
			base.OnEnter();
			this.duration = WindPrimary.baseDuration / this.attackSpeedStat;
				...
				if (base.isAuthority)
				{
					...
					for(int i = 0; i < projectileCount; i++)
					{
						...
						ProjectileManager.instance.FireProjectile(Modules.Projectiles.firePrimaryProjectile,
							...
							lerp,
							base.gameObject,
							WindPrimary.damageCoefficient * this.damageStat,
							4000f,
							base.RollCrit(),
							DamageColorIndex.Default,
							null,
							WindPrimary.throwForce);
					}
				}
			}
		}
